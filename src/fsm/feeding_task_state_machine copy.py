...
def generic_userdata_state_callback(userdata, response, next_state_on_success, next_state_on_failure, success_flag_value):
    ...
    if response.success:
        success_loginfo(f"{state_name}: success")
        userdata.ud_success = success_flag_value
        return next_state_on_success
    else:
        success_loginfo(f"{state_name}: failed")
        userdata.ud_success = not success_flag_value
        return next_state_on_failure
...
class food_item_selector(smach_ros.ServiceState):
    ...
    def execute(self, userdata):
        ...
        if userdata.success:
            userdata.ud_anygrasp_transform = userdata.ud_anygrasp_transforms[0]
            print('The best grasp from AnyGrasp: ')
            print(userdata.ud_anygrasp_transform)
            ...
            T_base_grasp = userdata.ud_anygrasp_transform 
...
def main():
    ...
    sm.userdata.ud_feeding_initial_position = feeding_initial_position
    # sm.userdata.ud_update_anygrasp = update_anygrasp

    # Open the container
    with sm:
        smach.StateMachine.add('move_to_feeding_initial_position'
                               , move_to_feeding_initial_position(sm.userdata.ud_feeding_initial_position
                                                  , input_keys_sm
                                                  , outcomes_sm)
